...
A string $s=pq$ if an \emph{anagram} if $p$ is a permutation of $q$.
...
Kamčev, Łuczak, and Sudakov \cite{kamcev.luczak.ea:XXX} and Wilson
and Wood \cite{wilson.wood:XXX} have shown that there are trees, and
...
chromatic number $O(k\log n)$.  Wilson and Wood showed that every tree
