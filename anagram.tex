...
\left\lceil\frac{k-1}{k-2}\right\rceil = 2 \enspace .
   ...
   There is some set $I_0\subset\Sigma$ such that $N(I_0)$ partitions
   $\{1,\ldots,n\}$ into $O(|I|/k)$ intervals.  One such interval
   $i_0,\ldots,j_0$ must have size $\Omega(nk/|I|)$.  By induction on $n$,
   $|\Sigma_{i_0,j_0}| = \Omega(k\log (nk/|I|))$.  But $\Sigma_{i_0,j_0}$
   is disjoint from $I$, so 
   \[  |\Sigma| \ge |I| + \Omega(k\log (nk/|I|))
        = |I| + \Omega(k\log n) - O(k\log(|I|/k))
