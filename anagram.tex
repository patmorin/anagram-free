...
\begin{proof}
   For any $1\le i\le j\le n$, let $\Sigma_{i,j}=\bigcup_{\ell=1}^j
   X_i$ and, for any $I\subset\Sigma_{i,j}$, let
   $N_{i,j}(I)=\{\ell\in\{i,\ldots,j\}: X_\ell\cap I\neq\emptyset \}$.

   Suppose that there is some pair $1\le i\le j$ such that, for every
...
\left\lceil\frac{k-1}{k-2}\right\rceil = 2 \enspace .
   ...
   set $I_\ell$.  Observe that, for $\ell \ge 1$,
   \[
        n_\ell \ge \frac{n_{\ell-1}-|N(I_{\ell-1})|}{|N(I_{\ell-1})|+1}
               > \frac{n_{\ell-1}}{|N(I_{\ell-1})|+1} - 1 \enspace .
   \]
   Letting $y_\ell=|N_{i_\ell,j_\ell}(I_\ell)|$, we  can easily show that
   \[
