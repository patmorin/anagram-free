...
using $\alpha <\sqrt{2\log_2 n}$ colours. From this we derive a string
$s=s_0\ldots,s_{n-1}$ where $s_i=\{\varphi(v_i),\varphi(w_i)\}$.
Observe that $s$ is a string over an alphabet $\Sigma$ of size
$c=\binom{\alpha}{2}$.  By \lemref{parity} and our choice of $\alpha$,
$s$ contains an even substring $s_i,\ldots,s_j$. Now, consider the path in $G$ of length $2n$ whose first half visits one of each twin pair so that, for each pair $\{a,b\}$, the first half of the path visits the vertex coloured $a$ exactly $n_{\{a,b\}}(s)/2$ times and visits the vertex coloured $b$ exactly $n_{\{a,b\}}(s)/2$ times. Now, the second half of the path visits one vertex in each 




 if $\binom{\alpha}{2} < \log_2(n+1)$.
